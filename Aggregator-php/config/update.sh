...
# upload.sh - 1.0  #
####################
REPOSITORY_NAME=aggregator-php.git
...
cd $GIT_DIR
(echo "FORCE SYNC WITH GITHUB.."; git fetch --all; git reset --hard origin/master; echo "SYNC DOCUMENT_ROOT.."; /usr/bin/rsync -rtvu --exclude '.git' --exclude 'application/config/database.php' --exclude 'stats'  --delete $GIT_DIR $DOCUMENT_ROOT);
