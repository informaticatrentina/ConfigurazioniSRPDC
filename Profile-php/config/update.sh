...
# upload.sh - 1.0  #
####################
REPOSITORY_NAME=profile-manager-php.git
            ...
            cd $GIT_DIR
                echo "INIZIALIZZO REPOSITORY GIT.."
                    git init
                        ...
                        cd $GIT_DIR
                        (echo "FORCE SYNC WITH GITHUB.."; git fetch --all; git reset --hard origin/master; echo "SYNC DOCUMENT_ROOT.."; /usr/bin/rsync -rtvu --exclude '.git' --exclude 'applica-tion/config/database.php' --exclude 'upload'  --delete $GIT_DIR $DOCUMENT_ROOT);
                        cp $PRIVATE_DIR/config.php $DOCUMENT_ROOT'/application/config'
                        cp $PRIVATE_DIR/ProfileManager.php $DOCUMENT_ROOT'/application/config'
